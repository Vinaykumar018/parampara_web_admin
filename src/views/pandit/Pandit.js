...
const { categories, loading: categoriesLoading, error: categoriesError } = useFetchCategories(panditIds);
  ...
  const handleAssignCategory = async (panditId, poojaIds) => {
    try {
      ...
      const selectedPoojas = poojaData.filter(p => poojaIds.includes(p._id));
  
      // Call API for each pooja assignment
      await Promise.all(
        poojaIds.map(poojaId => {
          const pooja = poojaData.find(p => p._id === poojaId);
          return axios.put(
            'http://34.131.10.8:3000/api/pandit/update-category',
            {
              pandit_id: panditId,
      ...
      // Update local state
      setPanditData(panditData.map(p => 
        p._id === panditId 
          ? { 
              ...p, 
              poojas: poojaIds.map(id => ({
                pooja_id: id,
                pooja_name: poojaData.find(p => p._id === id)?.pooja_name
              }))
            } 
          : p
      ));
  ...
  { 
    ...
    selector: (row) => {
      ...
      return categoryList.length > 0 ? categoryList.join(', ') : 'N/A';
    }, 
    sortable: true, 
    width: '500px' 
  },
  ...
  
    {
      ...
      selector: (row) => (
        <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'nowrap' }}>
          <Link to={`/pandit/${row._id}`} className="btn btn-info btn-sm text-white me-2">View</Link>
          
          <Link to={`/pandit/bookings/${row._id}`} className="btn btn-success btn-sm text-white me-2">Bookings</Link>  
...
>
  Manage Poojas
          ...
          <button onClick={() => handleEdit(row._id)} className="btn btn-primary btn-sm me-2">
            Edit
          </button>
  
          <button onClick={() => handleDelete(row._id)} className="btn btn-danger btn-sm me-2">
            Delete
          </button>      
        </div>
      ),
  ...
  <AssignCategoryModal
    ...
    poojaData={poojaData}
    currentPoojas={panditData.find(p => p._id === selectedPandit)?.poojas?.map(p => p.pooja_id) || []}
...
const AssignCategoryModal = ({ show, onClose, panditId, poojaData, onAssign, currentPoojas = [] }) => {
  const [selectedPoojas, setSelectedPoojas] = useState([]);
  const [loading, setLoading] = useState(false);

  // Initialize with currently assigned poojas
  useEffect(() => {
    if (currentPoojas) {
