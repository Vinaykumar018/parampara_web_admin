...
const Product = () => {
  const { globalContextProductData, setGlobalContextProductData } = useContext(AppContext);
        ...
        if (response?.success) {
            setProducts((prevProducts) =>
                prevProducts.map((product) =>
                    product._id === id ? { ...product, isFeatured: newStatus } : product
                )
            );
...
const handleStatusToggle = async (id) => {
    try {
        ...
        const newStatus = updatedProduct.status === "active" ? "inactive" : "active";
...
const columns = [
    { name: "S No.", selector: (row, index) => index + 1, grow: 0.3 },
    {
        name: "Featured Image",
        selector: (row) => (
            <img src={`http://localhost:3000/${row.featuredImage}`} alt={row.name} width={80} height={80} />
        ),
        grow: 1
    },
    ...
    },
    {
        ...
        selector: (row) => (
            <div style={{ display: "flex", gap: "8px", justifyContent: "center", flexWrap: "nowrap" }}>
                ...
                <button onClick={() => handleView(row)} className="btn btn-info btn-sm text-white">View</button>
            </div>
        ),
        ignoreRowClick: true,
        allowOverflow: true,
